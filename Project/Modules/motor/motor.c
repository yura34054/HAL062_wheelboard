...
void updatePID(int32_t encoder_ticks) {
	...
	current_speed = encoder_ticks * 1000 / (VELOCITY_CLOCK_TIME * ENC_PULSE_PER_ROTATION);
	current_speed *= 300; // 100 is 1 rotate per sec

//	TODO figure out if this is needed
//	low-pass filter
// 	if (current_speed != 0) {
// 		current_speed = LPF_ALPHA * current_speed + (1 - LPF_ALPHA) * (*prev_out);
// 		*prev_out = current_speed;
// 	} else {
// 		*prev_out = 0.0f;
//	}
